...
type ConnectionPool struct {
	...
	// Maximum number of connections per target (-1 = unlimited)
	maxConnections int
...
func NewConnectionPool(maxConnIdle time.Duration, minActiveConns, maxConnections int) *ConnectionPool {
...
func (p *ConnectionPool) Get(createFn func() (grpc.ClientConnInterface, error)) (conn grpc.ClientConnInterface, err error) {
	// Store the create function for dynamic creation
	p.lock.Lock()
	p.createFn = createFn
	p.lock.Unlock()
...
// doRegister performs the actual registration.
...
// If all connections are saturated, attempt to create a new one.
func (p *ConnectionPool) doShare() grpc.ClientConnInterface {
	...
	// Could not find a connection with less than 100 active streams
	// NEW: Try to create a new connection if we haven't hit the limit
	if (p.maxConnections < 0 || len(p.connections) < p.maxConnections) && p.createFn != nil {
		newConn, err := p.createFn()
		if err == nil && newConn != nil {
			p.doRegister(newConn)
			// Get the newly created connection with reference count increment
			if len(p.connections) > 0 {
				conn := p.connections[len(p.connections)-1]
				atomic.AddInt32(&conn.referenceCount, 1)
				return conn.conn
			}
		}
	}

	// Could not find a connection or create a new one, return nil
	return nil
}
...
func NewRemoteConnectionPool(maxConnections int) *RemoteConnectionPool {
...
func (p *RemoteConnectionPool) loadOrStoreItem(address string) *ConnectionPool {
	...
	if !ok {
		...
		item, _ = p.pool.LoadOrStore(address, NewConnectionPool(maxConnIdle, 0, p.maxConnections))
	}
